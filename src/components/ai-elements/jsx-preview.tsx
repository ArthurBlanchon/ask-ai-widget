...
export const JSXPreviewContent = memo(
  ({ className, ...props }: JSXPreviewContentProps) => {
    ...
    const [errorReportedJsx, setErrorReportedJsx] = useState<string | null>(
      null
    );
    ...
    if (processedJsx !== prevProcessedJsx) {
      ...
      setHadError(false);
      setErrorReportedJsx(null);
    }

    const handleError = useCallback(
      (err: Error) => {
        if (errorReportedJsx === processedJsx) {
          return;
        }
        setErrorReportedJsx(processedJsx);
      ...
      [processedJsx, errorReportedJsx, isStreaming, onErrorProp, setError]
    );
    ...
    }, [processedJsx, errorReportedJsx, setLastGoodJsx]);

    const displayJsx =
      isStreaming && hadError ? lastGoodJsx : processedJsx;
